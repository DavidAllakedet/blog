...
<article class="flex flex-col lg:flex-row pb-10 md:pb-16 border-b">
    ...
    <div class="flex flex-col items-start mt-5 space-y-5 lg:w-7/12 lg:mt-0 lg:ml-12">
        {{-- Affiche la catégorie du post s'il y en a une --}}
        @if($post->category)
        ...
        {{-- Affiche les tags du post s'il y en a --}}
        ...
        {{-- Affiche l'extrait ou le contenu complet du post --}}
        <p class="text-xl lg:text-2xl text-slate-600">
            @if($list)
                {{ $post->excerpt }} {{-- Affiche l'extrait --}}
            @else
                {!! nl2br(e($post->content)) !!} {{-- Affiche le contenu complet --}}
            @endif
        </p>

        {{-- Si $list est vrai, affiche le lien pour lire l'article complet --}}
        @if($list)
        ...
        {{-- Sinon, affiche la date de création du post --}}
        @else
